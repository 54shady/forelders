...
run_testcase()
{
	...
	if ! $1 ; then
		STATUS=1
		print "$script: FAIL test case: $1\n\n"
	else
		print "$script: PASS test case: $1\n\n"
	fi
...
expect_fail()
{
	...
	if ! $1 ; then
		print "$script: PASS test case returns fail as expected: $1\n\n"
	else
		STATUS=1
		print "$script: FAIL test case returns pass when it should have failed: $1\n\n"
	fi
...
check_devnode()
{
	...
	if [ ! -e $1 ]; then
		STATUS=1
		print "$script: FAIL devnode not found: $1\n\n"
	else
		print "$script: PASS devnode found: $1\n\n"
	fi
...
check_executable()
{
	...
	if [ ! -x $1 ]; then
		STATUS=1
		if [ -e $1 ]; then
			print "$script: FAIL executable found but not executable: $1\n\n"
			ls -lh $1
		else
			print "$script: FAIL executable not found: $1\n\n"
		fi
	else
		print "$script: PASS executable found: $1\n\n"
	fi
